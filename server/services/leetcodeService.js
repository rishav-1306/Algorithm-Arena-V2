 ...
 * Types come from metaData.params to guide parsing.
 */
 ...
 * Handles both inline and <pre>-block formats.
 */
const extractExpectedOutputs = (html) => {
  const results = [];
  // Primary: <strong>Output:</strong> value<newline-or-tag>
...
exports.fetchLeetCodeChallenges = async (slug) => {
  ...
  try {
    ...
    // ── 2. Parse metaData for function name + param info ───────────────────
    let functionName = "";
    let params = [];
    try {
      ...
      // leave empty — admin can fill manually
    }

    // ── 3. Build testCases from exampleTestcaseList + HTML expected outputs ─
